...
def reshape_group_additional(
    table: pd.DataFrame, split_string: str = " &&& "
) -> pd.DataFrame:
    ...
    group = exploded[["group_name", "group_level"]].pivot(
        columns="group_name", values="group_level"
    )
    additional = exploded[["additional_name", "additional_level"]].pivot(
    ...
    group_and_additional = (
        group.join(additional)
        .join(exploded["index"])
        .set_index("index")
        .groupby(level=0)
        .first()
...
def reshape_estimate_values(table: pd.DataFrame) -> pd.DataFrame:
    estimate_pivot = table.pivot(columns="estimate_name", values="estimate_value")
    no_estimates = table.drop(
        [
            "estimate_name",
            "estimate_type",
            "estimate_value",
    ...
    group_columns = list(no_estimates.columns)

    return (
        no_estimates.join(estimate_pivot)
